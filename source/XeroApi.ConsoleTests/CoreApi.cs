...
namespace XeroApi.ConsoleApp
{
    public class CoreApi
    {
        ...
        public void Exercise(CoreRepository repository)
        {
            if (repository == null)
            {
                return;
            ...
            TestFindingCustomersThatAreContacts(repository);
            TestGetOrganisationUsingLinqSingle(repository);
            ...
            TestAttachmentFromByteArray(repository, organisation);
            TestAttachmentsAgainstPurchaseInvoice(repository, organisation);
        ...
        private static void TestAttachmentsAgainstPurchaseInvoice(CoreRepository repository, Organisation organisation)
        {
            Console.WriteLine("Creating an invoice and loading attachment...");

            var anyPurchasesInvoice = repository.Invoices.FirstOrDefault(
                it => it.Type == "ACCPAY" && it.Status == "DRAFT")
                                      ?? CreateAnyPurchasesInvoice(repository, organisation);

            Console.WriteLine("Created invoice {0}", anyPurchasesInvoice.InvoiceID);
        ...
        private static void TestAttachmentFromByteArray(CoreRepository repository, Organisation organisation)
        {
            Console.WriteLine("Creating an invoice and loading attachment...");

            var anyPurchasesInvoice = CreateAnyPurchasesInvoice(repository, organisation);
        ...
        private static Invoice CreateAnyPurchasesInvoice(CoreRepository repository, Organisation organisation)
        {
            var invoice = new Invoice
            {
                ...
                LineItems = new LineItems
                {
                    new LineItem
                    {
                        ...
                        Quantity = 1,
                        UnitAmount = 1,
                        TaxType = GetNoTaxTypeForCountry(organisation),
                    }
                }
            };
        ...
        private static string GetNoTaxTypeForCountry(Organisation organisation)
        {
            if (organisation.CountryCode == "AU")
                return "BASEXCLUDED";
            return "NONE";
        }

        private static void TestGettingATrialBalance(CoreRepository repository)
        {
        ...
        private static void TestGettingAListOfExpenseClaims(CoreRepository repository)
        {
            // Get a list of all expense claims
            Console.WriteLine("Getting a list of all submitted expense claims...");

            foreach (
                var expenseClaim in repository.ExpenseClaims.Where(expenseClaim => expenseClaim.Status != "CURRENT"))
            {
                Console.WriteLine("Expense claim {0} for user {1} for amount {2} with status {3}",
                                  expenseClaim.ExpenseClaimID,
                                  expenseClaim.User.FullName, expenseClaim.Total, expenseClaim.Status);
            }
        }

        private static void TestCreatingReceiptsForAUser(CoreRepository repository)
        {
            ...
            {
                Console.WriteLine(
                    "Again, this is probably a demo organisation. There are no UserIDs returned for these organisations.");
                return;
            }
            ...
            {
                string message = string.Format("The receipt was not succesfully created: {0}",
                                               receipt.ValidationErrors.Select(it => it.Message)
                                                      .Aggregate((s1, s2) => string.Concat(s1, ";", s2)));
                throw new ApplicationException(message);
            }

            Console.WriteLine("Receipt {0} was created for {1} for user {2}", receipt.ReceiptID, receipt.Contact.Name,
                              receipt.User.FullName);


            // Upload an attachment against the newly creacted receipt
            var attachmentFileInfo = new FileInfo(AnyAttachmentFilename);
            ...
            {
                Console.WriteLine("The uploaded attachment filesize {0} does not match the original filesize {1}",
                                  attachment.ContentLength, attachmentFileInfo.Length);
            }
            else if (attachment.FileName != attachmentFileInfo.Name)
            {
                Console.WriteLine("The uploaded attachment filename '{0}' does not match the original filename '{1}'",
                                  attachment.FileName, attachmentFileInfo.Name);
            }
            else
            {
        ...
        private static void TestFindingTheSubscriberUser(CoreRepository repository)
        {
        ...
        private static void TestDownloadingPrintedInvoicePdf(CoreRepository repository, Invoice anySalesInvoice)
        {
            ...
            byte[] invoicePdf = repository.FindById<Invoice>(anySalesInvoice.InvoiceID.ToString(),
            ...
            var file = new FileInfo(invoicePath);
        ...
        private static void TestFindingInvoicesByContactName(CoreRepository repository, Contact contact)
        {
            ...
            var invoicesForContact =
                repository.Invoices.Where(invoice => invoice.Contact.ContactID == contact.ContactID).ToList();
            ...
            {
                Console.WriteLine("Invoice {0} was raised against {1} on {2} for {3}{4}",
                                  invoiceForContact.InvoiceNumber,
                                  invoiceForContact.Contact.Name, invoiceForContact.Date, invoiceForContact.Total,
                                  invoiceForContact.CurrencyCode);
            }
        }

        private static void TestCreatingInvoiceWithValidationErrors(CoreRepository repository)
        {
        ...
        private static void TestFindingItemsUsingLinqSyntax(CoreRepository repository, Organisation organisation)
        {
            ...
            {
                Console.WriteLine("   Item {0} is sold at price: {1} {2}", item.Description, item.SalesDetails.UnitPrice,
                                  organisation.BaseCurrency);
            }
        }

        private static void TestFindingTrackingCategories(CoreRepository repository)
        {
            ...
            {
                Console.WriteLine("Tracking Category: {0}", trackingCategory.Name);

                foreach (var trackingOption in trackingCategory.Options)
                {
                    Console.WriteLine("    Option: {0}", trackingOption.Name);
                }
            }
        }

        private static void TestFindingBankAccounts(CoreRepository repository)
        {
            ...
            Console.WriteLine("There were {0} bank accounts in this organisation.", bankAccounts.Count());

            foreach (var bankAaccount in bankAccounts)
            {
                Console.WriteLine("Bank Account Name:{0} Code:{1} Number:{2}", bankAaccount.Name,
                                  bankAaccount.Code, bankAaccount.BankAccountNumber);
            }
        }

        private static void TestFindingCustomersThatAreContacts(CoreRepository repository)
        {
            // Construct a linq expression to call 'GET Contacts'...
            var customers = repository.Contacts.Where(c => c.IsCustomer).ToList();

            Console.WriteLine("There are {0} contacts that are customers.", customers.Count);
        ...
        private static void TestGetContactsUsingLinq(CoreRepository repository)
        {
            // Construct a linq expression to call 'GET Contacts'...
            int invoiceCount = repository.Contacts.Count(c => c.UpdatedDateUTC >= DateTime.UtcNow.AddMonths(-1));

            Console.WriteLine("There were {0} contacts created or updated in the last month.", invoiceCount);
        }

        private static void TestCreatingAndUpdatingContacts(CoreRepository repository)
        {
            ...
            Console.WriteLine("The contact '{0}' was created with id: {1}", contact.Name, contact.ContactID);
            Console.WriteLine("The validation status was: {0}", contact.ValidationStatus);
            ...
            contact = repository.UpdateOrCreate(contact);
            Console.WriteLine("The contact '{0}' was updated with email address: {1}", contact.Name,
                              contact.EmailAddress);

            // Get the contact by it's Id...
            var reReadContact = repository.Contacts.First(c => c.ContactID == contact.ContactID);
            Console.WriteLine("The contact '{0}' was re-read using it's ContactID: {1}", reReadContact.Name,
                              reReadContact.ContactID);
        }

        private static void TestGetAllJournalsWithPagination(CoreRepository repository)
        {
            // Get all journals from the general ledger using the ?offset=xxx parameter
            var allJournals = new List<Journal>();
        ...
        private static void TestGetAccountsByFilter(CoreRepository repository)
        {
            // API v2.15 Get a list of accounts that can be used when creating expense claims
            var expenseClaimAccounts = repository.Accounts.Where(a => a.ShowInExpenseClaims).ToList();

            Console.WriteLine("There are {0} accounts that can be used in expense claim line items",
                              expenseClaimAccounts.Count);
        ...
        private static void TestGetOrganisationUsingLinqSingle(CoreRepository repository)
        {
        ...
        private static void TestGettingAListOfReports(CoreRepository repository)
        {
